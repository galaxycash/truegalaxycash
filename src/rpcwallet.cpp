...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new TrueGalaxyCash address for receiving payments.  "
    ...
    return CTrueGalaxyCashAddress(keyID).ToString();
}


CTrueGalaxyCashAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return CTrueGalaxyCashAddress(account.vchPubKey.GetID());
}
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current TrueGalaxyCash address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <TrueGalaxyCashaddress> <account>\n"
            "Sets the account associated with the given address.");

    CTrueGalaxyCashAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid TrueGalaxyCash address");
...
Value getaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getaccount <TrueGalaxyCashaddress>\n"
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    ...
    Array ret;
    BOOST_FOREACH(const PAIRTYPE(CTrueGalaxyCashAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CTrueGalaxyCashAddress& address = item.first;
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendtoaddress <TrueGalaxyCashaddress> <amount> [comment] [comment-to] [subtractfee]\n"
...
Value sendfromaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw runtime_error(
            "sendfromaddress <FromTrueGalaxyCashAddress> <ToTrueGalaxyCashAddress> <amount> [comment] [comment-to] [subtractfee]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());

    CTrueGalaxyCashAddress fromaddress(params[0].get_str());
    if (!fromaddress.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid TrueGalaxyCash address");

    CTrueGalaxyCashAddress toaddress(params[1].get_str());
...
Value listaddressgroupings(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        {
            Array addressInfo;
            addressInfo.push_back(CTrueGalaxyCashAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                LOCK(pwalletMain->cs_wallet);
                if (pwalletMain->mapAddressBook.find(CTrueGalaxyCashAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CTrueGalaxyCashAddress(address).Get())->second);
            }
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <TrueGalaxyCashaddress> <message>\n"
    ...
    CTrueGalaxyCashAddress addr(strAddress);
    if (!addr.IsValid())
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <TrueGalaxyCashaddress> [minconf=1]\n"
            "Returns the total amount received by <TrueGalaxyCashaddress> in transactions with at least [minconf] confirmations.");

    // Bitcoin address
    CTrueGalaxyCashAddress address = CTrueGalaxyCashAddress(params[0].get_str());
    CScript scriptPubKey;
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid TrueGalaxyCash address");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            "sendfrom <fromaccount> <toTrueGalaxyCashaddress> <amount> [minconf=1] [comment] [comment-to] [subtractfee]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());

    string strAccount = AccountFromValue(params[0]);
    CTrueGalaxyCashAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid TrueGalaxyCash address");
    int64_t nAmount = AmountFromValue(params[2]);
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    set<CTrueGalaxyCashAddress> setAddress;
    vector<pair<CScript, int64_t> > vecSend;

    int64_t totalAmount = 0;
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        CTrueGalaxyCashAddress address(s.name_);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid TrueGalaxyCash address: ")+s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress <nrequired> <'[\"key\",\"key\"]'> [account]\n"
            ...
            "each key is a TrueGalaxyCash address or hex-encoded public key\n"
    ...
    for (unsigned int i = 0; i < keys.size(); i++)
    {
        ...
        CTrueGalaxyCashAddress address(ks);
    ...
    return CTrueGalaxyCashAddress(innerID).ToString();
}
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    // Tally
    map<CTrueGalaxyCashAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CTrueGalaxyCashAddress, string)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CTrueGalaxyCashAddress, tallyitem>::iterator it = mapTally.find(address);
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; TrueGalaxyCash server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
Value makekeypair(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "makekeypair [compressed]\n"
            "Make a public/private key pair.\n"
            "[compressed] is optional compression flag for the public key.\n");

    bool fCompressed = false;
    if (params.size() > 0)
        fCompressed = params[0].get_bool();
    ...
    Object result;
    result.push_back(Pair("PrivateKey", CTrueGalaxyCashSecret(key).ToString()));
...
Value importaddress(const Array& params, bool fHelp)
{
...

    {
        ...
        if (address.IsValid()) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid TrueGalaxyCash address or script");
    }
    if (fRescan)
    {
