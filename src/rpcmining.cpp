...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    GenerateTrueGalaxyCashs(fGenerate, pwalletMain, nGenProcLimit);
    return Value::null;
}
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "TrueGalaxyCash is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "TrueGalaxyCash is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "TrueGalaxyCash is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "TrueGalaxyCash is downloading blocks...");
...
Value submitblock(const Array& params, bool fHelp)
{
    ...
    if (block.IsProofOfWork() && block.IsBadCoinbase())
        throw JSONRPCError(RPC_POW_LAST_BLOCK , "Bad coinbase, no masternode payments!");
