...
void Shutdown()
{
    ...
    if (pwalletMain)
        bitdb.Flush(true);
    GenerateTrueGalaxyCashs(false, NULL, 0);
#endif
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: truegalaxycash.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: truegalaxycashd.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 7604 or testnet: 17604)") + "\n";
...
#endif
    strUsage += "  -classic               " + _("Use the TrueGalaxyCash Classic network") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 4604 or testnet: 14604)") + "\n";
    ...
    strUsage += "\n" + _("SSL options: (see the TrueGalaxyCash Wiki for SSL setup instructions)") + "\n";
    strUsage += "  -rpcssl                                  " + _("Use OpenSSL (https) for JSON-RPC connections") + "\n";
    strUsage += "  -rpcsslcertificatechainfile=<file.cert>  " + _("Server certificate file (default: server.cert)") + "\n";
    strUsage += "  -rpcsslprivatekeyfile=<file.pem>         " + _("Server private key (default: server.pem)") + "\n";
    strUsage += "  -rpcsslciphers=<ciphers>                 " + _("Acceptable ciphers (default: TLSv1.2+HIGH:TLSv1+HIGH:!SSLv2:!aNULL:!eNULL:!3DES:@STRENGTH)") + "\n";
    strUsage += "  -nonotify                                " + _("Disable transaction notifications.") + "\n";
    return strUsage;
}

/** Sanity checks
 *  Ensure that TrueGalaxyCash is running in a usable environment with all
...
bool InitSanityCheck(void)
{
    if(!ECC_InitSanityCheck()) {
        InitError("OpenSSL appears to lack support for elliptic curve cryptography. For more "
                  "information, visit https://en.truegalaxycash.it/wiki/OpenSSL_and_EC_Libraries");
        return false;
    }
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. TrueGalaxyCash is shutting down."));
...
#endif
    // Make sure only a single TrueGalaxyCash process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. TrueGalaxyCash is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("TrueGalaxyCash version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "TrueGalaxyCash server starting\n");
    ...
    // requested to kill truegalaxycash-qt during the last operation. If so, exit.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of TrueGalaxyCash") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart TrueGalaxyCash to complete") << "\n";
    ...
    if (pwalletMain)
        GenerateTrueGalaxyCashs(GetBoolArg("-gen", false), pwalletMain, GetArg("-genproclimit", -1));
#endif
    // ********************************************************* Step 12: finished
