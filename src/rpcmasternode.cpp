...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  start        - Start masternode configured in TrueGalaxyCash.conf\n"
                "  start-alias  - Start single masternode by assigned alias configured in masternode.conf\n"
                "  start-many   - Start all masternodes configured in masternode.conf\n"
                "  status       - Print masternode status information\n"
                "  stop         - Stop masternode configured in TrueGalaxyCash.conf\n"
                ...
                "  vote-many    - Vote on a TrueGalaxyCash initiative\n"
                "  vote         - Vote on a TrueGalaxyCash initiative\n"
                );
    ...
    if (strCommand == "genkey")
    {
        ...
        return CTrueGalaxyCashSecret(secret).ToString();
    }
    ...
    if(strCommand == "vote-many")
    {
        ...
        BOOST_FOREACH(CMasternodeConfig::CMasternodeEntry mne, masternodeConfig.getEntries()) {
            ...
            if(!mnodeman.SetKey(mne.getPrivKey(), errorMessage, keyMasternode, pubKeyMasternode)){
    ...
    if(strCommand == "status")
    {
        ...
        for (int i = 0; i < activemnodeman.NumNodes(); i++)
        {
            ...
            mnObj.push_back(Pair("pubkey", CTrueGalaxyCashAddress(instance->pubkey.GetID()).ToString()));
...
Value masternodelist(const Array& params, bool fHelp)
{
                ...
                && strMode != "pubkey" && strMode != "rank" && strMode != "status" && strMode != "addr" && strMode != "votes" && strMode != "lastpaid"))
    {
        throw runtime_error(
                ...
                "  votes          - Print all masternode votes for a TrueGalaxyCash initiative (can be additionally filtered, partial match)\n"
