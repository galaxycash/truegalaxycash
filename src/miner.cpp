...
void ThreadStakeMiner(CWallet *pwallet)
{
    ...
    RenameThread("truegalaxycash-stake-miner");
...
void static TrueGalaxyCashMiner(CWallet *pwallet)
{
    LogPrintf("TrueGalaxyCashMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("truegalaxycash-miner");
    ...
    try { while (true) {
        ...
        LogPrintf("Running TrueGalaxyCashMiner with %llu transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("TrueGalaxyCashMiner terminated\n");
        throw;
    }
}

void GenerateTrueGalaxyCashs(bool fGenerate, CWallet* pwallet, int nThreads)
{
	...
	minerThreads->create_thread(boost::bind(&TrueGalaxyCashMiner, pwallet));
}
#endif
