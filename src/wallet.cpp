...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CTrueGalaxyCashAddress(redeemScript.GetID()).ToString();
    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    LogPrintf("WalletUpdateSpent found spent coin %s TGCH %s\n", FormatMoney(wtx.GetCredit()), wtx.GetHash().ToString());
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    LogPrintf("ReacceptWalletTransactions found spent coin %s TGCH %s\n", FormatMoney(wtx.GetCredit()), wtx.GetHash().ToString());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...
int64_t CWallet::GetAddressBalance(const CTrueGalaxyCashAddress &address) const
{
    int64_t nTotal = 0;
    {
        LOCK2(cs_main, cs_wallet);
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            if (pcoin->IsTrusted())
            {
                for (int i = 0; i < pcoin->vout.size(); i++)
                {
                    ...
                    {
                       CTrueGalaxyCashAddress address2(destination);
                       if (address == address2)
                           nTotal += txout.nValue;
                    }
                }
...
void CWallet::AvailableCoinsNoLocks(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl) const
{
    vCoins.clear();
...
// populate vCoins with vector of spendable COutputs
void CWallet::AvailableCoinsFrom(const CTxDestination &from, vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl) const
{
    vCoins.clear();

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                {
                    CTrueGalaxyCashAddress addr1(from);
                    CTrueGalaxyCashAddress addr2(destination);
                    if (!(addr1 == addr2))
                        continue;
                }
                else
...
bool CWallet::SelectCoinsFrom(const CTxDestination &from, int64_t nTargetValue, unsigned int nSpendTime, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64_t& nValueRet, const CCoinControl* coinControl) const
{
    vector<COutput> vCoins;
    AvailableCoinsFrom(from, vCoins, true, coinControl);
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                int64_t nMinFee = GetMinFee(nBestHeight, wtxNew, 1, GMF_SEND, nBytes);
...
bool CWallet::CreateTransactionSubfee(const vector<pair<CScript, int64_t> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl* coinControl)
{
    int64_t nValue = 0;
    unsigned int nSubtractFee = 0;
    BOOST_FOREACH (const PAIRTYPE(CScript, int64_t)& s, vecSend)
    {
        if (nValue < 0)
            return false;
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                BOOST_FOREACH (const PAIRTYPE(CScript, int64_t)& s, vecSend)
                {
                    ...
                    txout.nValue -= nFeeRet / nSubtractFee; // Subtract fee equally from each selected recipient

                    if (fFirst) // first receiver pays the remainder not divisible by output count
                    {
                        fFirst = false;
                        txout.nValue -= nFeeRet % nSubtractFee;
                    }
...
bool CWallet::CreateTransactionFrom(const CTxDestination &from, const vector<pair<CScript, int64_t> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl* coinControl)
{
    int64_t nValue = 0;
    BOOST_FOREACH (const PAIRTYPE(CScript, int64_t)& s, vecSend)
    {
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                if (!SelectCoinsFrom(from, nTotalValue, wtxNew.nTime, setCoins, nValueIn, coinControl))
                    return false;
...
bool CWallet::CreateTransactionFromSubfee(const CTxDestination &from, const vector<pair<CScript, int64_t> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl* coinControl)
{
    int64_t nValue = 0;
    unsigned int nSubtractFee = 0;
    BOOST_FOREACH (const PAIRTYPE(CScript, int64_t)& s, vecSend)
...
bool CWallet::CreateTransactionSubfee(CScript scriptPubKey, int64_t nValue, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl* coinControl)
{
    vector< pair<CScript, int64_t> > vecSend;
    vecSend.push_back(make_pair(scriptPubKey, nValue));
    return CreateTransactionSubfee(vecSend, wtxNew, reservekey, nFeeRet, coinControl);
}
...
bool CWallet::CreateTransactionFrom(const CTxDestination &from, CScript scriptPubKey, int64_t nValue, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl* coinControl)
{
    vector< pair<CScript, int64_t> > vecSend;
    vecSend.push_back(make_pair(scriptPubKey, nValue));
    return CreateTransactionFrom(from, vecSend, wtxNew, reservekey, nFeeRet, coinControl);
}

bool CWallet::CreateTransactionFromSubfee(const CTxDestination &from, CScript scriptPubKey, int64_t nValue, CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, const CCoinControl* coinControl)
{
    vector< pair<CScript, int64_t> > vecSend;
    vecSend.push_back(make_pair(scriptPubKey, nValue));
    return CreateTransactionFromSubfee(from, vecSend, wtxNew, reservekey, nFeeRet, coinControl);
}
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    if ( Params().NetworkID() == CChainParams::TEST ){
...
string CWallet::SendMoneySubfee(CScript scriptPubKey, int64_t nValue, CWalletTx& wtxNew, bool fAskFee)
{
    CReserveKey reservekey(this);
    int64_t nFeeRequired;
    ...
    if (!CreateTransactionSubfee(scriptPubKey, nValue, wtxNew, reservekey, nFeeRequired))
    {
        string strError;
        if (nValue + nFeeRequired > GetBalance())
            strError = strprintf(_("Error: This transaction requires a transaction fee of at least %s because of its amount, complexity, or use of recently received funds!"), FormatMoney(nFeeRequired));
...
string CWallet::SendMoneyFrom(const CTxDestination &from, CScript scriptPubKey, int64_t nValue, CWalletTx& wtxNew, bool fAskFee)
{
    CReserveKey reservekey(this);
    int64_t nFeeRequired;
    ...
    if (!CreateTransactionFrom(from, scriptPubKey, nValue, wtxNew, reservekey, nFeeRequired))
    {
        string strError;
        if (nValue + nFeeRequired > GetBalance())
            strError = strprintf(_("Error: This transaction requires a transaction fee of at least %s because of its amount, complexity, or use of recently received funds!"), FormatMoney(nFeeRequired));
...
string CWallet::SendMoneyFromSubfee(const CTxDestination &from, CScript scriptPubKey, int64_t nValue, CWalletTx& wtxNew, bool fAskFee)
{
    CReserveKey reservekey(this);
    int64_t nFeeRequired;
    ...
    if (!CreateTransactionFromSubfee(from, scriptPubKey, nValue, wtxNew, reservekey, nFeeRequired))
    {
        string strError;
        if (nValue + nFeeRequired > GetBalance())
            strError = strprintf(_("Error: This transaction requires a transaction fee of at least %s because of its amount, complexity, or use of recently received funds!"), FormatMoney(nFeeRequired));
...
string CWallet::SendMoneyToDestinationSubfee(const CTxDestination& address, int64_t nValue, CWalletTx& wtxNew, bool fAskFee)
{
    // Check amount
    if (nValue <= 0)
        return _("Invalid amount");
    ...
    return SendMoneySubfee(scriptPubKey, nValue, wtxNew, fAskFee);
}

string CWallet::SendMoneyFromToDestination(const CTxDestination& from, const CTxDestination& address, int64_t nValue, CWalletTx& wtxNew, bool fAskFee)
{
    // Check amount
    if (nValue <= 0)
        return _("Invalid amount");
    ...
    return SendMoneyFrom(from, scriptPubKey, nValue, wtxNew, fAskFee);
}

string CWallet::SendMoneyFromToDestinationSubfee(const CTxDestination& from, const CTxDestination& address, int64_t nValue, CWalletTx& wtxNew, bool fAskFee)
{
    // Check amount
    if (nValue <= 0)
        return _("Invalid amount");
    ...
    return SendMoneyFromSubfee(from, scriptPubKey, nValue, wtxNew, fAskFee);
}
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CTrueGalaxyCashAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CTrueGalaxyCashAddress(address).ToString());
}
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        {
            if (!IsLockedCoin(pcoin->GetHash(), n) && IsMine(pcoin->vout[n]) && pcoin->IsSpent(n) && (txindex.vSpent.size() <= n || txindex.vSpent[n].IsNull()))
            {
                LogPrintf("FixSpentCoins found lost coin %s TGCH %s[%d], %s\n",
            ...
            else if (!IsLockedCoin(pcoin->GetHash(), n) && IsMine(pcoin->vout[n]) && !pcoin->IsSpent(n) && (txindex.vSpent.size() > n && !txindex.vSpent[n].IsNull()))
            {
                LogPrintf("FixSpentCoins found spent coin %s TGCH %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue), pcoin->GetHash().ToString(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
...
void CWallet::AvailableCoinsMN(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl) const
{
    vCoins.clear();
