...
{
    QString ret = QString("truegalaxycash:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non TGCH unit input in lnReqAmount, we generate the URI with TGCH as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(TrueGalaxyCashUnits::format(TrueGalaxyCashUnits::TGCH, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
