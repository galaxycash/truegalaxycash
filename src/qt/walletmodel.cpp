...
WalletModel::WalletModel(CWallet *wallet, OptionsModel *optionsModel, QObject *parent) :
    ...
    cachedBalance(0), cachedStake(0), cachedUnconfirmedBalance(0), cachedImmatureBalance(0), cachedLockedBalance(0),
...
void WalletModel::checkBalanceChanged()
{
    ...
    if(cachedBalance != newBalance || cachedStake != newStake || cachedUnconfirmedBalance != newUnconfirmedBalance || cachedImmatureBalance != newImmatureBalance || cachedLockedBalance != newLockedBalance)
    {
...
{
    CTrueGalaxyCashAddress addressParsed(address.toStdString());
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(const QList<SendCoinsRecipient> &recipients, const CCoinControl *coinControl)
{
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            scriptPubKey.SetDestination(CTrueGalaxyCashAddress(rcp.address.toStdString()).Get());
...
{
    QString strAddress = QString::fromStdString(CTrueGalaxyCashAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        mapCoins[CTrueGalaxyCashAddress(address).ToString().c_str()].push_back(out);
    }
}
...
{
    LOCK2(cs_main, wallet->cs_wallet);
    return pwalletMain->IsLockedCoin(hash, n);
}
