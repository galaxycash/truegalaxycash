...
#include "truegalaxycashaddressvalidator.h"
...
QFont truegalaxycashAddressFont()
{
...
{
    widget->setMaxLength(TrueGalaxyCashAddressValidator::MaxAddressLength);
    widget->setValidator(new TrueGalaxyCashAddressValidator(parent));
    widget->setFont(truegalaxycashAddressFont());
}
...
bool parseTrueGalaxyCashURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("truegalaxycash"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!TrueGalaxyCashUnits::parse(TrueGalaxyCashUnits::TGCH, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseTrueGalaxyCashURI(QString uri, SendCoinsRecipient *out)
{
    // Convert truegalaxycash:// to truegalaxycash:
    //
    //    Cannot handle this later, because truegalaxycash:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("truegalaxycash://"))
    {
        uri.replace(0, 12, "truegalaxycash:");
    }
    QUrl uriInstance(uri);
    return parseTrueGalaxyCashURI(uriInstance, out);
}
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "TrueGalaxyCash.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for TrueGalaxyCash.lnk
...
{
    return GetAutostartDir() / "truegalaxycash.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a truegalaxycash.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=TrueGalaxyCash\n";
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = (tr("TrueGalaxyCash-Qt")) + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  truegalaxycash-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("TrueGalaxyCash-Qt"));
...
void SetWhiteThemeQSS(QApplication& app)
{
    ...
    app.setStyleSheet("QWidget        { background: rgb(230,230,230); }"
                      "QMenu::item:selected { background-color: rgb(205,205,205); }");
#else
