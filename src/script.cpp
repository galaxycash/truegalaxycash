...
CScriptNum CastToBigNum(const valtype& vch, const size_t nMaxNumSize = nDefaultMaxNumSize)
{
    if (vch.size() > nMaxNumSize)
        throw runtime_error("CastToBigNum() : overflow");
    // Get rid of extra leading zeros
    return CScriptNum(vch, false);
}
...
bool IsDERSignature(const valtype &vchSig, bool haveHashType) {
    // See https://truegalaxycashtalk.org/index.php?topic=8392.msg127623#msg127623
        ...
        while (pc < pend)
        {
            ...
            switch (opcode)
            {
                ...
                case OP_CHECKLOCKTIMEVERIFY:
                {
                    ...
                    const CScriptNum nLockTime = CastToBigNum(stacktop(-1), 5);
                ...
                case OP_HASH256:
                {
                    ...
                    if (opcode == OP_RIPEMD160)
                        Ripemd160Hash(&vch[0], vch.size(), &vchHash[0]);
                    else if (opcode == OP_SHA1)
                        Sha1Hash(&vch[0], vch.size(), &vchHash[0]);
                    else if (opcode == OP_SHA256)
                        Sha256Hash(&vch[0], vch.size(), &vchHash[0]);
