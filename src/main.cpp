...
#if defined(NDEBUG)
# error "TrueGalaxyCash cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "TrueGalaxyCash Signed Message:\n";
...
int64_t GetMinFee(const int32_t nHeight, const CTransaction& tx, unsigned int nBlockSize, enum GetMinFee_mode mode, unsigned int nBytes)
{
    ...
    int64_t nBaseFee = (mode == GMF_RELAY) ? GetMinTransactionFee(tx.nTime) : GetMinTransactionFee(tx.nTime);
...
bool CMerkleTx::AcceptToMemoryPool(bool fLimitFree, bool ignoreFees)
{
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    if (nHeight < 5000)
        nSubsidy = (1.25 * COIN);
    else if (nHeight < 230000 && nHeight >= 5000)
        nSubsidy = 1.0 * COIN;
    else if (nHeight < 360000 && nHeight >= 230000)
        nSubsidy = 0.96 * COIN;
    else if (nHeight < 560000 && nHeight >= 360000)
        nSubsidy = 0.86 * COIN;
    else if (nHeight < 720000 && nHeight >= 560000)
        nSubsidy = 0.76 * COIN;
    else if (nHeight < 1220000 && nHeight >= 720000)
        nSubsidy = 0.66 * COIN;
    else
        nSubsidy = 0.5 * COIN;
...
int64_t GetProofOfWorkReward(int64_t nFees, int nHeight)
{
    ...
    if (nHeight == 1)
        ...
        return (0.0001 * COIN) + nFees; // Anti instamine
...
const CBlockIndex* GetLastBlockIndex(const CBlockIndex* pindex, const bool fProofOfStake)
{
...
unsigned int DarkGravityWave(const CBlockIndex* pindexLast, const bool fProofOfStake)
{
    ...
    while (BlockReading && BlockReading->nHeight > 0) {
        ...
        if (BlockReading->IsProofOfStake() != fProofOfStake) {
    ...
    int64_t _nTargetTimespan = CountBlocks * Params().PowTargetSpacing(pindexLast->nHeight);
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, const bool fProofOfStake)
{
...
void TurnDownload(int nHeight)
{
    pindexLastBest = FindBlockByHeight(nHeight);
    nLastUpdate = pindexLastBest->GetBlockTime();
}
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags, bool fVerifySig)
{
    ...
    // fMiner is true when called from the internal truegalaxycash miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            {
                // Verify signature
                if (fVerifySig && !VerifySignature(txPrev, *this, i, flags, 0))
                {
        ...
        if (!IsCoinStake())
        {
            ...
            int64_t nRequiredFee = GetMinFee(nBestHeight, *this);
...
bool CTransaction::GetCoinAge(CTxDB& txdb, const CBlockIndex* pindexPrev, uint64_t& nCoinAge) const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
        ...
        LogPrint("coinage", "coin age nValueIn=%d nTimeDiff=%d bnCentSecond=%u\n", nValueIn, nTime - txPrev.nTime, bnCentSecond);
    }

    uint64_t bnCoinDay = bnCentSecond * CENT / COIN / (24 * 60 * 60);
    LogPrint("coinage", "coin age bnCoinDay=%u\n", bnCoinDay);
    nCoinAge = bnCoinDay;
    return true;
}
...
bool CBlock::IsBadCoinbase() const {
    ...
    // Check for PoW invalid masternode payment
    ...
    if (!fIsInitialDownload)
    {
        ...
        if(pindex != NULL){
                if(pindex->GetBlockHash() == hashPrevBlock){
                    ...
                    CTrueGalaxyCashAddress address2(address1);
...
bool CBlock::AcceptBlock()
{
    ...
    if (nVersion < MINIMAL_VERSION)
        return DoS(100, error("AcceptBlock() : reject too old nVersion = %d", nVersion));
    else if (nVersion > CURRENT_VERSION)
    ...
    // Check Coinbase
    if (IsProofOfWork() && IsBadCoinbase())
        return error("ProcessBlock() : Bad coinbase, no masternode payments!");
    ...
    if (GetNextTargetRequired(pindexPrev, IsProofOfStake()) != nBits)
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Check Coinbase
    if (pblock->IsProofOfWork() && pblock->IsBadCoinbase())
        return error("ProcessBlock() : Bad coinbase, no masternode payments!");
...
{
    RenameThread("truegalaxycash-loadblk");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    else if (strCommand == "getcoins")
    {
        if (GetBoolArg("-coins", true)) {
            ...
            for (unsigned int i = 0; i < num; i++) {
                uint160 address; uint256 hash; std::vector <COutPoint> coins;
                vRecv >> address;
                vRecv >> hash;

                if (!pCoins->CheckHash(address, hash)) {
                    pCoins->GetSpendableCoins(address, coins);
                    pfrom->PushMessage("coins", address, hash, coins);
                }
    ...
    else if (strCommand == "gettxes")
    {
        if (GetBoolArg("-coins", true)) {
            ...
            for (unsigned int i = 0; i < num; i++) {
                uint160 address; uint256 hash; std::vector <COutPoint> coins; std::vector <CTransaction> txes;
                vRecv >> address;
                vRecv >> hash;
                ...
                if (!pCoins->CheckHash(address, hash)) {
                    ...
                    for (unsigned int j = 0; j < coins.size(); j++) {
                        ...
                        if (GetTransaction(coins[j].hash, tx, block)) {
                            std::vector <CTransaction>::iterator it = std::find(txes.begin(), txes.end(), tx);
                            if (it == txes.end())
                                txes.push_back(tx);
                        }
                    }
                    pfrom->PushMessage("txes", address, hash, txes);
                }
            }
        }
    }

    else if (strCommand == "txes")
    {
    }
    ...
    else if (strCommand == "tx")
    {
        ...
        if (AcceptToMemoryPool(mempool, tx, true, &fMissingInputs, ignoreFees))
        {
